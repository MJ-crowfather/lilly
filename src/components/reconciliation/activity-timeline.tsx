        ...
        <div className="max-w-4xl mx-auto">
            <div className="relative">
                ...
                {activities.map((activity, index) => {
                    const activityDate = new Date(activity.timestamp).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
                    const showDate = activityDate !== lastDate;
                    if (showDate) {
                    ...
                    return (
                        <React.Fragment key={activity.id}>
                            ...
                            <div className="flex items-start gap-4 mb-8">
                                <div className="text-xs text-muted-foreground min-w-[50px] text-right mt-1">{new Date(activity.timestamp).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}</div>
                                <div className="relative">
                                    <div className={cn(
                                        "h-4 w-4 rounded-full border-2 flex items-center justify-center bg-background",
                                        activity.status === 'completed' ? 'border-green-500' : 'border-gray-300'
                                    )}>
                                       {activity.status === 'completed' && <div className="h-1.5 w-1.5 bg-green-500 rounded-full" />}
                                    </div>
                                </div>
